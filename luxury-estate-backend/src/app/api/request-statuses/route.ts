...
export async function GET(req: NextRequest) {
  try {
    ...
    const statuses = await prisma.requestStatus.findMany({ where: { tenantId }, orderBy: { sortOrder: 'asc' } });
...
export const POST = requireAuth()(async (req: NextRequest) => {
  try {
    ...
    const tenantId = getTenantId(req)!;
    const status = await prisma.requestStatus.create({ data: { tenantId, ...body } });
